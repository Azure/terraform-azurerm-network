...
resource "azurerm_subnet" "subnet_count" {
  ...
  private_endpoint_network_policies_enabled      = lookup(var.subnet_enforce_private_endpoint_network_policies_enabled, var.subnet_names[count.index], false)
...
resource "azurerm_subnet" "subnet_for_each" {
  ...
  resource_group_name                            = var.resource_group_name
  virtual_network_name                           = azurerm_virtual_network.vnet.name
  private_endpoint_network_policies_enabled      = lookup(var.subnet_enforce_private_endpoint_network_policies_enabled, each.value, false)
